...
public abstract class Scene {
    ...
    //TODO:
    // - Able to remove objects dynamically
    // It is dangerous to blindly remove objects during iteration,
    // We want to do this after iteration so that it is stable.
    // - Able to add objects dynamically
    // same reason, objects should also get added AFTER the adding-iteration
    // has happened.
    ...
    public List<T> GetObjectsOfType<T>() where T : FComponent {
        ...
        if (objects.IsEmpty) 
            Logger.Log(this, $"No object in {GetType().Name} has a single component of {typeof(T).Name}", LogType.Warn); 
